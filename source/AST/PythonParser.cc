            ...
            if ((for_offset >= 0) && (for_offset < end_offset))
            {
                ...
                if (is_dict)
                {
                    return shared_ptr<Expression>(new DictComprehension(key_pattern,
                                                                        item_pattern, variable, source_data, predicate,
                                                                        offset));
                }
        ...
        ssize_t end_offset)
{
    ...
    static const vector<TokenType> augment_operator_tokens = {
            TokenType::_PlusEquals, TokenType::_MinusEquals, TokenType::_AsteriskEquals, TokenType::_SlashEquals,
            TokenType::_PercentEquals,
            TokenType::_AndEquals, TokenType::_OrEquals, TokenType::_XorEquals, TokenType::_LeftShiftEquals,
        ...
        ssize_t end_offset)
{
    ...
    while (this->token_num < end_offset)
    {
        ...
        switch (this->head_token().type)
        {
            ...
            { // ReturnStatement; expect an optional expression
                this->advance_token();
            ...
            { // RaiseStatement; expect an optional expression
                this->advance_token();
            ...
            case TokenType::Exec:
            { // ExecStatement; expect an expression, or two or three
                this->advance_token();
                auto exprs = this->parse_expression_list(line_end_offset);
            ...
            { // AssertStatement; expect an expression, or two
                this->advance_token();
                auto exprs = this->parse_expression_list(line_end_offset);
            ...
            case TokenType::If:
            { // IfStatement; expect an expression, colon, then suite
                clear();
                this->advance_token();
            ...
            case TokenType::Else:
            { // ElseStatement; expect a colon, then a suite
                expect_else();
            ...
            case TokenType::Elif:
            { // ElifStatement; expect an expression, colon, then suite
            ...
            { // WhileStatement; expect an expression, colon, then suite
                clear();
                this->advance_token();
            ...
            case TokenType::For:
            { // ForStatement; expect _Dynamic list, In, expr, colon, suite
                clear();
                this->advance_token();
            ...
            case TokenType::Try:
            {  // TryStatement; expect a colon, then suite
                clear();
                this->advance_token();
                this->expect_token_type(TokenType::_Colon, ParseError::SyntaxError,
            ...
            { // FinallyStatement; expect a colon, then quite
                expect(false, false, false, false, true); // try-statement only
