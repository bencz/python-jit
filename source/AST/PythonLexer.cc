...
bool PythonLexer::Token::is_operator_token(TokenType type)
{
    static const set<TokenType> operator_tokens({
                                                        ...
                                                        TokenType::_LessThan, TokenType::_GreaterThan,
                                                        TokenType::_Equality,
                                                        TokenType::_GreaterOrEqual, TokenType::_LessOrEqual,
                                                        TokenType::_NotEqual,
                                                        TokenType::Is, TokenType::IsNot, TokenType::_Or,
                                                        TokenType::_Xor,
...
PythonLexer::PythonLexer(shared_ptr<const SourceFile> source) : source(source)
{
    ...
    while (position < len)
    {
        ...
        if ((token.type == TokenType::_InvalidToken) &&
            ((str[0] == 'b') || (str[0] == 'u') || (str[0] == 'B') || (str[0] == 'U')) &&
            (!strncmp(str + 1, "\'\'\'", 3) || !strncmp(str + 1, "\"\"\"", 3)))
        {
            ...
            TokenType type = ((str[0] == 'b') || (str[0] == 'B')) ? TokenType::_BytesConstant
                                                                  : TokenType::_UnicodeConstant;
        ...
        switch (token.type)
        {
            ...
            case TokenType::_Newline:
                ...
                if (next_indent > indent_levels[indent])
                {
                    ...
                    while (next_indent < indent_levels[indent])
                    {
