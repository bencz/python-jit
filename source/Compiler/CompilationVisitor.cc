...
CompilationVisitor::CompilationVisitor(GlobalContext *global,
                                       ModuleContext *module, Fragment *fragment) : file_offset(-1),
                                                                                    ...
                                                                                    available_int_registers(
                                                                                            default_available_int_registers),
                                                                                    available_float_registers(
                                                                                            default_available_float_registers),
                                                                                    target_register(rax),
                                                                                    float_target_register(xmm0),
                                                                                    stack_bytes_used(0),
...
void CompilationVisitor::visit(UnaryOperation *a)
{
    ...
    switch (a->oper)
    {
        ...
        case UnaryOperator::Positive:
            ...
            {
                throw compile_error("arithmetic positive operator can only be applied to numeric values",
                                    this->file_offset);
            }
            break;
        ...
        case UnaryOperator::Negative:
            ...
            {
                throw compile_error("arithmetic negative operator can only be applied to numeric values",
                                    this->file_offset);
            }
            break;
    ...
    switch (a->oper)
    {
        ...
        case BinaryOperator::NotIn:
            ...
            else
            {
                // TODO
                throw compile_error(
                        "operator `in` not yet implemented for " + left_type.str() + " and " + right_type.str(),
                        this->file_offset);
            }
        ...
        case BinaryOperator::Or:
            ...
            throw compile_error("operator `or` not valid for " + left_type.str() + " and " + right_type.str(),
                                this->file_offset);
        ...
        case BinaryOperator::And:
            ...
            throw compile_error("operator `and` not valid for " + left_type.str() + " and " + right_type.str(),
                                this->file_offset);
        ...
        case BinaryOperator::Xor:
            ...
            throw compile_error("operator `xor` not valid for " + left_type.str() + " and " + right_type.str(),
                                this->file_offset);
        ...
        case BinaryOperator::RightShift:
            ...
            throw compile_error("bit shift operator not valid for " + left_type.str() + " and " + right_type.str(),
                                this->file_offset);
        ...
        case BinaryOperator::Addition:
            ...
            {
                throw compile_error(
                        "addition operator not implemented for " + left_type.str() + " and " + right_type.str(),
                        this->file_offset);
            }
            break;
        ...
        case BinaryOperator::Subtraction:
            ...
            {
                throw compile_error(
                        "subtraction operator not implemented for " + left_type.str() + " and " + right_type.str(),
                        this->file_offset);
            }
            break;
        ...
        case BinaryOperator::Multiplication:
            ...
            {
                throw compile_error(
                        "multiplication operator not implemented for " + left_type.str() + " and " + right_type.str(),
                        this->file_offset);
            }
            break;
        ...
        case BinaryOperator::Division:
        {
            ...
            {
                throw compile_error(
                        "division operator not implemented for " + left_type.str() + " and " + right_type.str(),
                        this->file_offset);
            }
        ...
        case BinaryOperator::Exponentiation:
            ...
            // TODO
            throw compile_error(
                    "exponentiation operator not implemented for " + left_type.str() + " and " + right_type.str(),
                    this->file_offset);
...
void CompilationVisitor::visit(FunctionCall *a)
{
    ...
    try
    {
        ...
        string call_split_label = string_printf(
                "__FunctionCall_%p_call_function_%" PRId64 "_fragment_%" PRId64 "_split_%" PRId64,
        ...
        if (collection_type.type == ValueType::Dict)
        {
            ...
            if (!this->current_type.types_equal(collection_type.extension_types[0]))
            {
                ...
                throw compile_error(string_printf("lookup for key of type %s on Dict[%s, %s]",
                                                  expr_key_type.c_str(), dict_key_type.c_str(),
                                                  dict_value_type.c_str()),
                                    this->file_offset);
            }
            if (type_has_refcount(this->current_type.type) && !this->holding_reference)
            {
    ...
    if (!cls->destructor)
    {
        ...
        else
        {
            ...
            if (del_index >= 0)
            {
                ...
                {
                    throw compile_error("__del__ exists but is not a function; instead it\'s " + del_attr.value.str(),
                                        this->file_offset);
                }
                                              ...
                                              bool setup_special_regs)
{
    ...
    size_t arg_stack_offset =
            sizeof(int64_t) * (setup_special_regs ? 6 : 2); // account for ret addr, rbp, and maybe special regs
...
void CompilationVisitor::write_delete_held_reference(const MemoryReference &mem)
{
    if (this->holding_reference)
    {
        if (!type_has_refcount(this->current_type.type))
        {
            throw compile_error("holding a reference to a trivial type: " + this->current_type.str(),
                                this->file_offset);
        }
        ...
        const string &name)
{
    ...
    VariableLocation loc;
    loc.name = name;
    loc.variable_mem = MemoryReference(rbp, sizeof(int64_t) * (-static_cast<ssize_t>(1 + distance(
            this->fragment->function->locals.begin(), it))));
    loc.variable_mem_valid = true;
